...
def generate_html(animals, animal_name):
    ...
    if animals:
        ...
        for animal in animals:
            animal_html += f"<h2>{animal['name']}</h2>"
            animal_html += "<ul>"
            ...
            animal_html += "</ul><hr>"
        final_html = template.replace("{{content}}", animal_html)
    else:
        final_html = template.replace("{{content}}", f"<h2>The animal \"{animal_name}\" doesn't exist.</h2>")
